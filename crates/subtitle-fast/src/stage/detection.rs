...
pub use strategy::{DefaultSubtitleBandStrategy, SubtitleBandStrategy};
...
mod model {
    ...
    use subtitle_fast_validator::subtitle_detection::{
        DetectionRegion, RoiConfig, SubtitleDetectionError,
    };
...
pub use model::{SegmentDebugInfo, SubtitleSegment, SubtitleStageError, SubtitleStageResult};
...
mod debug {
    ...
    use subtitle_fast_validator::subtitle_detection::{DetectionRegion, RoiConfig, SubtitleDetectionResult};

    #[derive(Default)]
    pub(super) struct DebugOutputs {
    ...
    impl DebugOutputs {
        ...
        pub(super) async fn record_frame(
            &mut self,
            frame: &YPlaneFrame,
        ...
        pub(super) async fn finalize(&mut self) -> std::io::Result<()> {
            if let Some(json) = self.json.as_mut() {
                json.flush().await?;
            }
            Ok(())
...
mod worker {
    ...
    use crate::settings::{ImageDumpSettings, JsonDumpSettings};
    use crate::stage::sampler::{FrameHistory, SampledFrame, SamplerContext};
    use subtitle_fast_decoder::YPlaneFrame;
    use subtitle_fast_validator::subtitle_detection::{
    ...
    use super::debug::DebugOutputs;
    use super::model::{SegmentDebugInfo, SubtitleSegment, SubtitleStageError};
    use super::strategy::SubtitleBandStrategy;
    use super::STABILITY_IOU_THRESHOLD;
    ...
    pub(super) struct SubtitleDetectionWorker {
        pub(super) fast_validator: FrameValidator,
        pub(super) precise_validator: FrameValidator,
        ...
        pub(super) processed_samples: u64,
        debug: DebugOutputs,
    }
    ...
    impl SubtitleDetectionWorker {
        ...
        pub(super) async fn finalize_outputs(&mut self) -> std::io::Result<()> {
        ...
        pub(super) async fn finalize_active(&mut self) -> Option<SubtitleSegment> {
        ...
        async fn determine_start(
            ...
            let window_frames: Vec<u64> =
                self.window.iter().map(|obs| obs.shot.frame_index).collect();
            ...
            for track in tracks {
                if let Some(last) = track.regions.last() {
                    if last.region.score > best_score {
                        best_score = last.region.score;
                        best_shot = Some(last.clone());
                    }
                }
            }

            let mut best_shot = best_shot
                .map(|selected| {
                    let mut shot = last_shot.clone();
                    let region = selected.region.clone();
                    shot.region = region.clone();
                    shot.score = region.score;
                    shot.regions = vec![region];
                    shot
                })
                .unwrap_or_else(|| last_shot.clone());

            for track in tracks {
                for entry in &track.regions {
                    if entry.frame_index == best_shot.frame_index && entry.region.score > best_shot.score
                    {
                        best_shot.region = entry.region.clone();
                        best_shot.score = entry.region.score;
                        best_shot.regions = vec![entry.region.clone()];
                    }
                }
            }
    ...
    fn roi_with_margin(region: DetectionRegion, width: u32, height: u32) -> RoiConfig {
        let margin = super::ROI_EXPANSION_PX;
...
mod pipeline {
    ...
    use crate::settings::{ImageDumpSettings, JsonDumpSettings};
    use crate::stage::sampler::SamplerResult;
    use crate::stage::StreamBundle;
    use subtitle_fast_validator::FrameValidator;

    use super::model::{SubtitleStageError, SubtitleStageResult};
    use super::strategy::SubtitleBandStrategy;
    use super::worker::SubtitleDetectionWorker;
    ...
    impl SubtitleDetectionStage {
        ...
        pub fn attach(self, input: StreamBundle<SamplerResult>) -> StreamBundle<SubtitleStageResult> {
            let StreamBundle {
                stream,
                total_frames,
            } = input;
...
pub use pipeline::SubtitleDetectionStage;
