...
Texture2D<float> t_y_plane : register(t2);
Texture2D<float2> t_uv_plane : register(t3);
...
SurfaceVertexOutput surface_vertex(uint vertex_id: SV_VertexID, uint surface_id: SV_InstanceID) {
    ...
    // Texture coords: 0,0 to 1,1 mapping to the bounds
    float2 texture_coords = unit_vertex * float2(1.0, 1.0); 
...
float4 surface_fragment(SurfaceFragmentInput input): SV_Target {
    float y = t_y_plane.Sample(s_sprite, input.texture_coords);
    float2 uv = t_uv_plane.Sample(s_sprite, input.texture_coords);
