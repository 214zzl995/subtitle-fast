...
use crate::{
    ...
    FileDropEvent, FontId, Global, GlobalElementId, GlyphId, GpuSpecs, Hsla, ImageId, InputHandler,
...
impl Window {
    ...
    pub fn paint_image(
        ...
        let scale_factor = self.scale_factor();
        let bounds = bounds.scale(scale_factor);
        if let Some(frame) = data.nv12_frame() {
            let content_mask = self.content_mask().scale(scale_factor);
            self.next_frame.scene.insert_primitive(PaintSurface {
                order: 0,
                bounds,
                content_mask,
                source: PaintSurfaceSource::Nv12 {
                    image_id: data.id,
                    frame: frame.clone(),
                },
            });
    ...
    pub fn paint_surface(&mut self, bounds: Bounds<Pixels>, image_buffer: CVPixelBuffer) {
        ...
        self.next_frame.scene.insert_primitive(PaintSurface {
            ...
            bounds,
            content_mask,
            source: PaintSurfaceSource::CvPixelBuffer(image_buffer),
        });
    }
    ...
    /// Removes an NV12 image from the renderer cache.
    pub fn drop_nv12_image(&mut self, image_id: ImageId) {
        self.platform_window.drop_nv12_image(image_id);
    }
